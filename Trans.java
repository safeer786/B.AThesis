...
public class Trans {
	...
	Boolean is_exists(String word, State current) {
		...
		for (int i = 1; i < word_length + 1; i++) {
			String subword = word.substring(0, i);
				if (current.transi.contains(subword)) {
					Transition transition = current.transitions.get(current.transi.indexOf(subword));
					if (word.substring(i).isEmpty()) {
						if (all.get(allnames
								.indexOf(transition.state)).isfinal) {
							result= true;
							break;
						} else {
							result = false;
						...
						result = result
								|| is_exists(word.substring(i),
										all.get(allnames
												.indexOf(transition.state)));
					}
				}
		}
	...
	List<String> upfinal(String word) {
		...
		List<String> output = new ArrayList<String>(); // keep record of actual
														// word
		List<String> c_output = new ArrayList<String>(); // keep record of
															// change output
		...
		if (output.isEmpty()) {
			...
			final_output = update_final(final_output, output, indexf); // without
																		// change
			ch_output = update_chh(ch_output, c_output, indexf); // with change
		}
		output.clear();
		final_output = Repe1(final_output, output, temp, word, ch_output); // we
																			// returned
																			// ch
																			// so
																			// now
	...
	List<String> Repe1(List<String> final_output, List<String> output,
			List<Transition> transition, String word, List<String> ch_output) {
		Transition t = new Transition("", "", 0.0, "", 0);
		List<Transition> temp = new ArrayList<Transition>();
	...
	List<String> update_chh(List<String> ch_output, List<String> output,
			int index) {
		List<String> temp = new ArrayList<String>();
